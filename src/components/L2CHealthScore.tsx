...
const STAGES = [
  { label: "Attract", score: 82, color: "#0B7F5C", icon: "📣" },
  { label: "Convert", score: 68, color: "#60A5FA", icon: "🎯" },
  { label: "Close", score: 71, color: "#108000", icon: "🤝" },
  { label: "Collect", score: 64, color: "#E17000", icon: "💰" },
  { label: "Grow", score: 75, color: "#3CBFA4", icon: "📈" },
];
...
function ScoreRing({ score, size = 100 }: { score: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      />
      <text x={size / 2} y={size / 2 - 6} textAnchor="middle" fontSize="28" fontWeight="800" fill="#22262A">{score}</text>
      <text x={size / 2} y={size / 2 + 14} textAnchor="middle" fontSize="10" fill="#9CA3AF">/100</text>
    </svg>
  );
}
  ...
  return (
    <div className="qbo-card p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[13px] font-semibold text-[#22262A]">L2C Health Score</h3>
          ...
          {STAGES.map((s) => {
            ...
            return (
              <div key={s.label} className="text-center">
                ...
                <p className="text-[10px] font-semibold text-[#22262A] mt-0.5">{s.label}</p>
