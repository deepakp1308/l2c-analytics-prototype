...
export function SceneTabs({ active, onChange }: { active: number; onChange: (id: number) => void }) {
  ...
  return (
    <div className="fixed bottom-0 left-[56px] right-0 bg-white/95 backdrop-blur-sm border-t border-[#E5E7EB] z-40">
      <div className="flex items-center h-10 px-3 overflow-x-auto gap-1">
        <span className="text-[9px] font-bold text-[#9CA3AF] uppercase tracking-widest px-1 shrink-0">
          Demo
        </span>
        ...
        {Object.entries(groups).map(([group, items]) => (
          <div key={group} className="flex items-center gap-0.5 shrink-0">
            <span className="text-[8px] font-bold text-[#9CA3AF] uppercase tracking-wider pl-1.5 pr-0.5">
              {group}
            </span>
            {items.map((s) => (
              <button
                ...
                className={`text-[11px] font-medium px-2 py-1 rounded transition-all ${
                  active === s.id
                    ...
                    : "text-[#6B7280] hover:bg-[#F4F5F8] hover:text-[#212B36]"
                }`}
              >
                {s.id}. {s.label}
              </button>
