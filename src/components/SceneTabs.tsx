...
export function SceneTabs({ active, onChange }: { active: number; onChange: (id: number) => void }) {
  ...
  return (
    <div className="fixed bottom-0 left-[56px] right-0 bg-white/95 backdrop-blur-sm border-t border-[#D1D5DB] z-40">
      <div className="flex items-center h-10 px-3 overflow-x-auto gap-1">
        <span className="text-[9px] font-bold text-[#9CA3AF] uppercase tracking-widest px-1 shrink-0">Demo</span>
        <div className="w-px h-4 bg-[#D1D5DB] mx-1" />
        {Object.entries(groups).map(([group, items]) => (
          <div key={group} className="flex items-center gap-0.5 shrink-0">
            <span className="text-[8px] font-bold text-[#9CA3AF] uppercase tracking-wider pl-1.5 pr-0.5">{group}</span>
            {items.map((s) => (
              <button
                key={s.id}
                ...
                className={`text-[11px] font-medium px-2 py-1 rounded transition-all ${
                  active === s.id
                    ? "bg-[#0B7F5C] text-white"
                    : "text-[#6B7280] hover:bg-[#F3F4F6] hover:text-[#22262A]"
                }`}
              >
                {s.id}. {s.label}
              </button>
