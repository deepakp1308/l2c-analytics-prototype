...
const ITEMS = [
  ...
  { label: "Pipeline (30d est.)", value: "$34K", color: "#0B7F5C", pct: 35 },
];
  ...
  return (
    <div className="qbo-card p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-[13px] font-semibold text-[#22262A]">Working Capital Impact</h3>
      </div>

      <div className="text-center mb-4">
        <p className="text-[10px] text-[#9CA3AF] uppercase">Projected 30-Day Cash Position</p>
        <p className="text-[28px] font-bold text-[#22262A]">$96K</p>
      ...
      <div className="space-y-2">
        {ITEMS.map((item) => (
          <div key={item.label} className="flex items-center justify-between">
            ...
            <span className="text-[12px] font-semibold text-[#22262A]">{item.value}</span>
          </div>
        ))}
      </div>

      <div className="mt-3 pt-3 border-t border-[#E5E7EB]">
        <p className="text-[10px] text-[#9CA3AF]">If AR is collected on time and pipeline closes at 55% rate</p>
      </div>
    </div>
  );
}
